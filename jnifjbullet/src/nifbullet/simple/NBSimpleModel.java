...
public class NBSimpleModel extends BranchGroup implements PartedBulletNifModel {
	...
	public void addPart(String partFileName, MeshSource meshSource, Object pointer, Transform3D rootTrans) {
		if (partFileName != null && partFileName.length() > 0) {
			...
			if (bulletNifModelClassifier.isStaticModel() || bulletNifModelClassifier.isKinematicModel()) {
				...
				if (niToJ3dData.nifVer.LOAD_VER < NifVer.VER_10_0_1_0) {
					//morrowind only
					boolean hasRootCollision = false;
					for (NiObject niObject : niToJ3dData.getNiObjects()) {
						if (niObject instanceof RootCollisionNode) {
							...
							if (!hasPivot) {
								NBStaticRigidBody nbbco = new NBStaticRigidBody(rootCollisionNode,
								...
								NBKinematicRigidBody kb = new NBKinematicRigidBody(this, doorPivot, rootCollisionNode,
					...
					// no root collision just turn all shapes into physics
					if (hasRootCollision == false) {
						for (NiObject niObject : niToJ3dData.getNiObjects()) {
					...
					for (NiObject niObject : niToJ3dData.getNiObjects()) {
						if (niObject instanceof bhkNPCollisionObject) {
							...
							if (nio instanceof bhkPhysicsSystem) {
								...
								NBStaticRigidBody nbbco = new NBStaticRigidBody(bhkNPCollisionObject, bhkPhysicsSystem,
										niToJ3dData.getNiObjects(), rootTrans, this, bodyId);
							...
							if (nio instanceof bhkRigidBody) {
								...
								if (layer == OblivionLayer.OL_ANIM_STATIC) {
									...
									NBKinematicRigidBody kb = new NBKinematicRigidBody(this, j3dNiNodeRoot,
											bhkCollisionObject, niToJ3dData, rootTrans, this, sf);
