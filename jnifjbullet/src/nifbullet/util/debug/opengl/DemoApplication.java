...
import static nifbullet.util.debug.opengl.IGL.GL_AMBIENT;
import static nifbullet.util.debug.opengl.IGL.GL_DEPTH_TEST;
import static nifbullet.util.debug.opengl.IGL.GL_DIFFUSE;
import static nifbullet.util.debug.opengl.IGL.GL_LESS;
import static nifbullet.util.debug.opengl.IGL.GL_LIGHT0;
import static nifbullet.util.debug.opengl.IGL.GL_LIGHT1;
import static nifbullet.util.debug.opengl.IGL.GL_LIGHTING;
import static nifbullet.util.debug.opengl.IGL.GL_MODELVIEW;
import static nifbullet.util.debug.opengl.IGL.GL_POSITION;
import static nifbullet.util.debug.opengl.IGL.GL_PROJECTION;
...
public abstract class DemoApplication
{
	...
	static
	{
		...
		{
			startTransforms[i] = NifBulletUtil.newIdentityTransform();
		}
	}
	...
	public void myinit()
	{
		...
		float[] light_position0 = new float[]
		{ 1.0f, 10.0f, 1.0f, 0.0f };
		float[] light_position1 = new float[]
		{ -1.0f, -10.0f, -1.0f, 0.0f };
		...
		switch (button)
		{
			...
			case 1:
			{
				if (state == 0)
				{
					...
					if (dynamicsWorld != null)
					{
						CollisionWorld.ClosestRayResultCallback rayCallback = new CollisionWorld.ClosestRayResultCallback(cameraPosition,
								rayTo);
	...
	protected float showProfileInfo(float xOffset, float yStart, float yIncr)
	{
...

		{
			...
			FastFormat.append(buf,
					(float) (parent_time > BulletGlobals.FLT_EPSILON ? ((parent_time - accumulated_time) / parent_time) * 100 : 0.f), 3);
			buf.append(" %) :: ");
	...
	private final Transform m = NifBulletUtil.newIdentityTransform();
	...
	public void renderme()
	{
		...
		if (dynamicsWorld != null)
		{
			...
			if ((debugMode & DebugDrawModes.NO_HELP_TEXT) == 0)
			{
				...
				drawString(buf, Math.round(xOffset), Math.round(yStart), TEXT_COLOR);
				yStart += yIncr;
				drawString("Cam pos = " + cameraPosition.toString(), Math.round(xOffset), Math.round(yStart), TEXT_COLOR);
				yStart += yIncr;
				drawString("Cam target pos = " + cameraTargetPosition.toString(), Math.round(xOffset), Math.round(yStart), TEXT_COLOR);
				yStart += yIncr;
		...
		for (int i = 0; i < numObjects; i++)
		{
			...
			if (body != null)
			{
				...
				dynamicsWorld.getBroadphase().getOverlappingPairCache()
						.cleanProxyFromPairs(colObj.getBroadphaseHandle(), getDynamicsWorld().getDispatcher());
